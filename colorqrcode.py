...
class ColorQRCode:
	...
	def get_data(self):
		...
		print "Done: ",
		print("%s seconds" % (time.time() - start_time))
		print "Original Size: " + str(len(temp_data))
		if self.filetype != 'txt':
			print "Converted Size: " + str(len(temp))
...
def main():
	if action in ACTIONS:
		...
		for i in xrange(1):
			...
			logs.append("SAMPLE " + str(i+1) + ": " + str(time.time()-start_time))
		with open('PERFLOG.txt', 'w+') as log_file:
			for log in logs:
